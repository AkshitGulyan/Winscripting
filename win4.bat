...
for /f "tokens=* delims=" %%I in (%IP_LIST%) do (
    ...
    echo Detecting USB mount point...
    for /f "tokens=* delims=" %%M in ('sdb -s !IP! shell "mount | grep vfat | awk "{print $3}""') do (
        set REMOTE_DIR=%%M
    )

    echo Mountpoint detected: !REMOTE_DIR!

    echo Getting file list from !IP!...
    sdb -s !IP! shell "for f in !REMOTE_DIR!/*; do case \"!f!\" in */Coredump*.gz|*/log_dump*.gz) echo $f ;; esac; done" > temp_files.txt
